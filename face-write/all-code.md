...
## 用promise如何解决第一个ajax返回的结果作为第二个ajax的参数，第二个ajax返回的结果作为第三个ajax的参数？
...
//如果下一个then要获取上一个ajax返回的参数，上一个then中的resolve必须得返回一个promise，也就是getJSON返回的是一个promise, 并且在ajax的success中调用resolve，传入ajax返回的结果。
getJSON("/post/1.json").then(function(post) {
    return getJSON(post.commentURL);
}).then(function(tem) {
    return getJSON(tem.URL);
}).then(function(res) {
...
## // 设计一个函数clickOnce，对指定的dom对象绑定click事件，但要求无论点击多少次，只有第一次的点击才触发事件回调callback
